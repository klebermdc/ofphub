...
const MarketingDashboard = () => {
  ...
  const fileCategories = ["NF Fornecedor", "NF Google", "NF Meta", "Comprovante", "Contrato", "Outro"];

  useEffect(() => {
    if (!loading && !roleLoading) {
      if (!user) navigate("/auth");
      else if (role === 'salesperson') navigate("/vendedor");
    }
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center p-8 glass rounded-xl max-w-md">
          ...
          <p className="text-muted-foreground mb-4">Sua conta ainda não foi vinculada pelo gestor.</p>
          <Button variant="outline" onClick={handleSignOut}><LogOut className="h-4 w-4 mr-2" />Sair</Button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-6 py-6 relative space-y-6">
        <MarketingTab costs={costs} onSave={saveCost} getCostForMonth={getCostForMonth} salesReps={salesReps} />
        ...
        <div className="glass rounded-xl p-6">
          ...
          <div className="flex flex-wrap items-center gap-4 mb-6">
            ...
            <input ref={fileInputRef} type="file" className="hidden" onChange={handleFileUpload} accept=".pdf,.png,.jpg,.jpeg,.doc,.docx,.xls,.xlsx" />
            <Button onClick={() => fileInputRef.current?.click()} disabled={isUploading} className="gap-2">
              <Upload className="h-4 w-4" />{isUploading ? "Enviando..." : "Upload de Arquivo"}
            </Button>
          </div>
          {files.length === 0 ? (
                    ...
                    <TableRow key={file.id}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-2">
                          <Button variant="ghost" size="icon" onClick={() => downloadFile(file.file_path, file.file_name)}>
                            <Download className="h-4 w-4" />
                          </Button>
                          <Button variant="ghost" size="icon" onClick={() => deleteFile(file.id, file.file_path)}>
